# Agent setup for `librebook_flutter`

This is a deepsec scanning workspace. Project `librebook_flutter` was just registered
(target: `..`). Setup is incomplete — `data/librebook_flutter/INFO.md`
...
2. **Fill in `data/librebook_flutter/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id librebook_flutter
pnpm deepsec process --project-id librebook_flutter
```
