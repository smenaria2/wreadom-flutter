...
Currently configured project: `librebook_flutter` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/librebook_flutter/SETUP.md` to fill in
   `data/librebook_flutter/INFO.md`.
...
`--project-id librebook_flutter` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/librebook_flutter/`.
