...
class FirebaseFeedRepository implements FeedRepository {
  ...
  Future<PagedResult<FeedPost>> getFollowingFeedPage(
    ...
    try {
      ...
      final queryResults = <List<FeedPost>>[];
      for (
        var index = 0;
        index < chunks.length;
        index += _followingFeedMaxParallelQueries
      ) {
        final window = chunks.sublist(
          index,
          math.min(index + _followingFeedMaxParallelQueries, chunks.length),
        );
        queryResults.addAll(
          await Future.wait(
            window.map((chunk) async {
              try {
                return await _fetchFollowingFeedChunk(
                  chunk,
                  limit: limit,
                ...
                final code = error is FirebaseException
                    ...
                    : 'unexpected';
                debugPrint(
                  '[FirebaseFeedRepository] Following chunk failed '
                  'code=$code',
                );
      ...
      for (final post in queryResults.expand((posts) => posts)) {
        byId[post.id ?? '${post.userId}:${post.timestamp}'] = post;
      }
      final allPosts = byId.values.toList();

      allPosts.sort((a, b) => b.timestamp.compareTo(a.timestamp));
      StartupPerformance.mark(
        'query_merge',
        page: 'feed_following',
        elapsed: mergeStopwatch.elapsed,
      );
        ...
        snapshot.docs
            .map((doc) {
              try {
                ...
                debugPrint(
                  '[FirebaseFeedRepository] Error parsing question answer '
                  '${doc.id}: $error',
                );
                return null;
              }
  ...
  Future<void> createFeedPost(FeedPost post) async {
    final enriched = await _backfillCachedBookFields(post);
  ...
  Future<FeedPost> _backfillCachedBookFields(FeedPost post) async {
    final bookId = post.bookId?.toString().trim();
    final needsTitle = post.bookTitle?.trim().isNotEmpty != true;
    final needsAuthor = post.bookAuthorName?.trim().isNotEmpty != true;
    if (bookId == null || bookId.isEmpty || (!needsTitle && !needsAuthor)) {
      return post;
    }
    try {
      final result = await FirestoreCacheFirst.document(
        _firestore.collection('books').doc(bookId),
        operation: 'feed_post_book_backfill',
      );
      final snapshot = result.cacheAvailable
          ? result.cached
          : await result.refresh;
      final raw = snapshot?.data();
      if (snapshot == null || !snapshot.exists || raw == null) return post;
      final book = Book.fromJson(normalizeBookMapForModel(raw, snapshot.id));
      final authorName = book.authors
          .map((author) => author.name.trim())
          .where((name) => name.isNotEmpty)
          .join(', ');
      return post.copyWith(
        bookTitle: needsTitle ? book.title : post.bookTitle,
        bookAuthorName: needsAuthor && authorName.isNotEmpty
            ? authorName
            : post.bookAuthorName,
        bookCover: post.bookCover?.trim().isNotEmpty == true
            ? post.bookCover
            : book.coverUrl,
      );
    } catch (_) {
